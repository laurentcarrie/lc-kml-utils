#!/usr/bin/env python3
"""Fetch all metro lines in Île-de-France and create KML files with routes and stops."""
...
def make_kml(line_name, long_name, operator, color, shapes, stops):
    """Create a KML file for a metro line."""
    kml = ET.Element("kml", xmlns="http://www.opengis.net/kml/2.2")
    doc = ET.SubElement(kml, "Document")
    ET.SubElement(doc, "name").text = f"Métro {line_name} - {long_name}"
    if operator:
    ...
    if stops:
        ...
        ET.SubElement(stops_folder, "name").text = "Stations"
        seen = set()
        for stop in stops:
...
def main():
    ...
    # Step 1: Fetch all metro line traces
    print("Fetching metro line traces...")
    traces = fetch_all_pages(TRACES_API, "route_type%3D%22Metro%22")
    print(f"  Got {len(traces)} metro line traces")

    if not traces:
        # Try alternative name
        print("  Trying 'Subway' filter...")
        traces = fetch_all_pages(TRACES_API, "route_type%3D%22Subway%22")
        print(f"  Got {len(traces)} traces with 'Subway' filter")

    if not traces:
        # Try Tramway too
        print("  Trying 'Tram' filter...")
        traces = fetch_all_pages(TRACES_API, "route_type%3D%22Tram%22")
        print(f"  Got {len(traces)} traces with 'Tram' filter")
    ...
    print(f"  {len(traces_by_id)} unique metro lines")

    # Step 2: Fetch metro stops in batches
    print("Fetching metro stops...")
    stops_by_id = {}
    session = requests.Session()
    session.verify = False
    import urllib3
    ...
    for rid, shapes in sorted(traces_by_id.items()):
        ...
        filename = f"metro-{sanitize_filename(short_name)}.kml"
